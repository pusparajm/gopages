...
var (
	...
	method       = `func Render%s(conn *http.Conn, request *http.Request){
					print := func(toPrint string){
						io.WriteString(conn, toPrint)
					}
...
func NewPage(page string) (p *Page, err os.Error) {
	p = new(Page)
	p.page = page
	f, err := os.Open(page, os.O_RDONLY, 0666)
	f.Close()
	return
}
...
func AddHandlers(pages []string) (err os.Error) {
	...
	for i, j, k := 0, 0, 1000; j < len(pages); i++ {
		...
		buffer.Append(fmt.Sprintf(init, st, handlers.Content(), initOthers(k, i)))
